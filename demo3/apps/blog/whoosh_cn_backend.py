...
class WhooshSearchBackend(BaseSearchBackend):
    ...
    def setup(self):
        ...
        # Make sure the indexes is there.
        if self.use_file_storage and not os.path.exists(self.path):
            os.makedirs(self.path)
            new_index = True

        if self.use_file_storage and not os.access(self.path, os.W_OK):
            raise IOError("The path to your Whoosh indexes '%s' is not writable for the current user/group." % self.path)
            ...
            try:
                ...
                try:
                    ...
                    self.log.error(u"%s while preparing object for update" % e.__class__.__name__,
                                   exc_info=True, extra={"data": {"indexes": index,
    ...
    def clear(self, models=None, commit=True):
        ...
        try:
            ...
            if models is not None:
                self.log.error("Failed to clear Whoosh indexes of models '%s': %s", ','.join(models_to_delete),
                               e, exc_info=True)
            else:
                self.log.error("Failed to clear Whoosh indexes: %s", e, exc_info=True)

    def delete_index(self):
        # Per the Whoosh mailing list, if wiping out everything from the indexes,
        # it's much more efficient to simply delete the indexes files.
